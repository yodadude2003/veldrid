    ...
    public class ImageSharpCubemapTexture
    {
        ...
        public unsafe Texture CreateDeviceTexture(GraphicsDevice gd, ResourceFactory factory)
        {
            ...
            for (int level = 0; level < MipLevels; level++)
            {
                ...
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelSpanNegX))
                {
                    throw new VeldridException("Unable to get negatve x pixelspan.");
                }
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelSpanPosY))
                {
                    throw new VeldridException("Unable to get positive y pixelspan.");
                }
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelSpanNegY))
                {
                    throw new VeldridException("Unable to get negatve y pixelspan.");
                }
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelSpanPosZ))
                {
                    throw new VeldridException("Unable to get positive z pixelspan."); 
                }
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelSpanNegZ))
                {
